...
def compute_kmedmbr(hyp=None, score_function=None, matrix=None, weights=None, src=None, k=1):
    ...
    kmedoids = KMedoids(n_clusters=k, metric=dist_func, method='pam', init='k-medoids++').fit(X)
...
if __name__ == "__main__":
    ...
    from utils import score_dir, output_dir
    ...
    for sample_id in tqdm(range(n_lines)):
        ...
        score_path = os.path.join(score_dir, dataset, '{:04d}_eps-{:.2f}_{}'.format(sample_id, eps, metric))
        ...
        output_path = os.path.join(output_dir, dataset, '{:04d}_{}'.format(sample_id, strategy_name))
