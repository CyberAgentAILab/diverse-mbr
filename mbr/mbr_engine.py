...
if __name__ == "__main__":
    ...
    for filename in filtered_files:
        ...
        try:
            ...
            print(os.path.join(sample_dir, filename), 'is not readable with default engine.')
            continue
        ...
        if algorithm != "dbs":
            ...
            df = df[:n_samples]
        df.fillna("", inplace=True) # TODO: This is needed to remove empty strings. In reality empty strings can be ignored. probably it's better to drop.
        hyp = df.iloc[:]['text']
        ...
        if algorithm == 'incremental':
            ...
            kmbr_scores = [compute_score(df, kmbr_bests[i], trg, compute_evaluate, src=src_input) for i in range(diverse_k)]
            kmbr_stats = evaluate_diversity(kmbr_hyps, kmbr_scores, src_input, compute_pairwise)

            # K-medoid MBR
            kmmbr_bests = compute_kmedmbr(matrix=matrix, k=diverse_k)
            kmmbr_hyps = df['text'].iloc[kmmbr_bests].to_list()
            kmmbr_scores = [compute_score(df, kmmbr_bests[i], trg, compute_evaluate, src=src_input) for i in range(diverse_k)]
            kmmbr_stats = evaluate_diversity(kmmbr_hyps, kmmbr_scores, src_input, compute_pairwise)

            row = [sample_id, kmbr_scores, kmbr_bests] + kmbr_stats + \
                    [kmmbr_scores, kmmbr_bests] + kmmbr_stats

            # Diverse MBR
            for diverse_pen in diverse_pens:
                dmbr_bests = compute_dmbr(matrix=matrix, k=diverse_k, div_pen=diverse_pen)
                dmbr_hyps = df['text'].iloc[dmbr_bests].to_list()
                dmbr_scores = [compute_score(df, dmbr_bests[i], trg, compute_evaluate, src=src_input) for i in range(diverse_k)]
            ...
            dbs_scores = [compute_score(df, i, trg, compute_evaluate, src=src_input) for i in range(diverse_k)]
            dbs_stats = evaluate_diversity(dbs_hyps, dbs_scores, src_input, compute_pairwise)
    ...
    if algorithm == 'incremental':
        ...
        columns = ['sample_id'] 
        methods = ["kmbr", 'kmmbr'] + ['dmbr-{}'.format(div_pen) for div_pen in diverse_pens]
        metrics = ["_score", "_best", "_mean_score", "_min_score", "_max_score", "_self_score", "_dn_1", "_dn_2", "_dn_3"]
