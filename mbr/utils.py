    ...
    if model_name == "None":
        ...
        if "polylm" in model_n:
            tokenizer = AutoTokenizer.from_pretrained(
                base_model_name, legacy=False, use_fast=False, padding_size="left"
            )
            if "polylm" in model_n:
...
def load_kwargs(dataset):
    ...
    E2E_KWARGS = dict(
        max_new_tokens=40, min_new_tokens=5, no_repeat_ngram_size=3
    )  # This is set by me but seems not optimal at all.
