...
def load_model(dataset, torch_device, model_name):
    ...
    if model_name == "None":
        if 'wmt19' in dataset:
            ...
            if 'wmt21fs' in dataset:
                ...
                if src_lang == 'en':
                    ...
                    model = AutoModelForSeq2SeqLM.from_pretrained(mname, load_in_4bit=True, device_map="auto")
                    tokenizer = AutoTokenizer.from_pretrained(mname)
                    model.config.forced_bos_token_id = tokenizer.get_lang_id(dataset.split('.')[1].split('-')[1])                
                else:
        ...
        elif dataset == 'chatbot_arena_instructions-en-ja-s2s':
            mname = "facebook/wmt21-dense-24-wide-en-x"
        ...
        if 'Mixtral' in model_n:
            ...
            model = AutoModelForCausalLM.from_pretrained(base_model_name, trust_remote_code=True, load_in_8bit=True, device_map="auto")
        elif ('Mistral' in base_model_name) or ('zephyr' in base_model_name) or ('mistral-7b-sft-beta' in base_model_name):
            model = AutoModelForCausalLM.from_pretrained(base_model_name, trust_remote_code=True, load_in_4bit=True, device_map="auto")
        else:
            model = AutoModelForCausalLM.from_pretrained(base_model_name, trust_remote_code=True, load_in_8bit=True, device_map="auto")
...
def load_dataset(dataset, ref=False, raw_text=False):
    ...
    if not (subdir_name == "hf"):
        ...
        if dataset == 'samsum':
            ...
            dataset = datasets.load_dataset("common_gen", split='validation')
            if not ref:
                lines = []
                idx = 0
                for row in dataset:
                    if row['concept_set_idx'] == idx:
                        lines.append(list_to_text(row['concepts']))
                        idx += 1
            else:
                lines = []
                idx = 0
                ...
                for row in dataset:
                    if row['concept_set_idx'] == idx:
                        cur_target.append(row['target'])
                    else:
                        lines.append(cur_target)
                        idx += 1
            ...
            inputs_sorted = list(set(dataset['context']))
            if not ref:
                inputs = {}
                for row in dataset:
                    context = row['context']
                ...
                for row in dataset:
                    context = row['context']
                    if context in targets.keys():
                        targets[context].append(row['question'])
                    else:
                        targets[context] = [row['question']]
                lines = list(targets.values())
        else:
            assert False
