...
def sample(dataset, tokenizer, model, src_lines, torch_device, n_lines, start_iter, n_samples, bsz, 
    ...
    for sample_id in tqdm(range(start_iter, n_lines)):
        ...
        # TODO: Prompts are depedent to both the model and the dataset.  This makes refactoring tricky... 
        if prompt == "None":
...
ASSISTANT: """
            ...
            if ('japanese-large-lm-3.6b-instruction-sft' in model_n) or ('llm-jp' in model_n) or ('dpo_20240122-153719' in model_n):
                # TODO: LINE's tokenizer put EOS at the end of the sentence. This is a hack to remove it.
                model_inputs = tokenizer(input_source, return_tensors="pt", 
                                    return_token_type_ids=False, add_special_tokens=False).to(model.device)                
            else:
                model_inputs = tokenizer(input_source, return_tensors="pt", 
